...
// Identify which of a wallet's Token-2022 mints are xStocks, via Jupiter's token search.
export async function GET(req: Request) {
  ...
  for (let i = 0; i < mints.length; i += 20) {
    ...
    for (const t of (await res.json()) as { id: string; symbol: string; name: string; decimals: number }[])
      if (/xstock/i.test(t.name)) out.push({ mint: t.id, symbol: t.symbol, name: t.name, decimals: t.decimals });
  }
