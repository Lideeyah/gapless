...
export async function POST(req: Request) {
  ...
  if (!DECIMAL.test(b.floor_price_usd!) || Number(b.floor_price_usd) <= 0 || b.floor_price_usd!.length > 24) return NextResponse.json({ error: "floor_price_usd must be a positive decimal string" }, { status: 400 });
